...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const avatarLocalPath = req.files?.avatar[0]?.path; // hum yahan pe 2 files le rhe hai coverImage and Avatar therefore "files" else use "file" for single file
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  //NOTE: if we need to update files(like img, video etc)
  // keep a seperate controller always
  ...
  const user = User.findByIdAndUpdate(
    req.user?._id,
    { $set: { fullname, email } },
    { new: true } // this will return updated info
  ).select("-password");
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  const avatarLocalPath = req.file?.path; // file bcz single file
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  const coverImageLocalPath = req.file?.path; // file
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  //Assignment aggregation/pipeline
  const channel = await User.aggregate([
    {
      $match: { username: username?.toLowerCase() }, // we match user
    },
    {
      $lookUp: {
        // counted how many subscriber of this user
        from: "subscriptions",
        localField: "_id",
    ...
    {
      $lookUp: {
        // counted how many channel user had subscribed
        from: "subscriptions",
        localField: "_id",
    ...
    {
      $addFields: {
        // original user object me 3-extra info added
        subscribersCount: {
          // no of subscribers
          $size: "$subscribers",
        },
        channelsSubscribedToCount: {
          // no of channel this user is subscribed
          $size: "$subscribedTo",
        },
        isSubscribed: {
          // curr channel is subscribed or not
          $condition: {
    ...
    {
      $project: {
        // this is used to pass only selected things
        fullname: 1,
        username: 1,
        subscribersCount: 1,
        ...
        coverImage: 1,
        email: 1,
        cretaedAt: 1, // this will help us to find when user created acc
      },
    },
  ]);
